...
2. Read state file `docs/slack/check-log.md` and determine the search window:
   a. If the file exists, extract `last_checked_at` timestamp (YYYY-MM-DD portion)
      - Compute the gap: run `date +%Y-%m-%d` to get today's date and compare with the `last_checked_at` date
      - If the gap is **3 days or less**: use the `last_checked_at` date as `target_date`
      - If the gap is **more than 3 days**: ask the user how far back to search (suggest the `last_checked_at` date as default). Use their answer as `target_date`
   b. If the file does not exist (first run): ask the user how far back to search (suggest "yesterday"). Use their answer as `target_date`
   c. Compute `filter_date`: the day before `target_date` using `date -v-1d -j -f "%Y-%m-%d" "<target_date>" +%Y-%m-%d` via Bash
      - **Why?** `filter_date_after` is exclusive (excludes the specified date). Passing the day before ensures messages from `target_date` are included
3. Search DMs: `conversations_search_messages` with `search_query: "to:me"` and `filter_date_after: "<filter_date>"`
4. Search channel mentions: `conversations_search_messages` with `search_query: "@<slack_username>"` and same `filter_date_after`
5. Search participating threads: `conversations_search_messages` with `filter_users_with: "@<slack_username>"`, `filter_threads_only: true`, and same `filter_date_after`. This finds new messages in threads where the user has participated, even without @mentions
