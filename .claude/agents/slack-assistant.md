...
tools: Read, Write, TaskCreate, mcp__slack__conversations_history, mcp__slack__conversations_replies, mcp__slack__conversations_add_message, mcp__slack__conversations_search_messages, mcp__slack__conversations_mark, mcp__slack__channels_list, mcp__slack__reactions_add, mcp__slack__reactions_remove, mcp__slack__users_search, mcp__slack__usergroups_list, mcp__slack__usergroups_me
skills:
  - slack-communication
---
...
1. **CHECK** (`/check-slack`): Search for DM messages (`to:me`) and channel mentions (`@<username>` resolved dynamically) to find items needing attention. Do NOT use `conversations_unreads` -- it is too slow on large workspaces. Return a structured task list to the main session
...
5. For each "requires action" item, determine ball ownership by checking the last message in the thread/DM:
   - **Case 1** (user's message last -- opponent has the ball): Create a TaskCreate for a reminder check (follow up if no response)
   - **Case 2** (opponent's message last -- user has the ball): Create a TaskCreate to discuss and draft a response
   - **Case 3** (conversation completed -- no further action needed): Ask the user to confirm marking as read
6. Present the full list (task items + awareness items) to the main session
